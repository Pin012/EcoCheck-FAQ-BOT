...
function findFaqAnswer(userQuery: string): FAQItem | null {
  ...
  // Similar FAQ match (strict):
  // 1) 需至少命中 2 個 keyword，或
  // 2) 命中 1 個「長 keyword（>=4字）」
  // 避免問任何問題都被 FAQ 誤攔截。
  let best: { faq: FAQItem; keywordHits: number; hasLongKeyword: boolean } | null = null;
  for (const faq of faqs) {
    const hitKeywords = (faq.keywords || []).filter((kw) => kw && userQuery.includes(kw));
    const keywordHits = hitKeywords.length;
    const hasLongKeyword = hitKeywords.some((kw) => kw.length >= 4);

    const eligible = keywordHits >= 2 || hasLongKeyword;
    if (!eligible) continue;

    if (!best || keywordHits > best.keywordHits) {
      best = { faq, keywordHits, hasLongKeyword };
    }
  }
